...
class GetOtherPofolService {
    ...
    fun getFollowPortfolio(jwt: String, pofolIdx: Int) {
        getPofolService?.getFollowPofol(jwt, pofolIdx)?.enqueue(object : Callback<GetPofolResponse> {
            override fun onResponse(call: Call<GetPofolResponse>, response: Response<GetPofolResponse>) {
                // 응답이 왔을 때 처리
                Log.d("GETPOFOL / SUCCESS", response.toString())
                ...
                when(val code = resp.code) {
                    1000 -> getPofolView.onGetFollowSuccess(resp.result)  // 성공
                    else -> getPofolView.onGetFollowFailure(code, resp.message)
                }
            }
