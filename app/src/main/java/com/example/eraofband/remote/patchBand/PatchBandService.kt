...
class PatchBandService {
    ...
    fun patchBand(jwt: String, bandIdx: Int, band : Band) {
        ...
        makeService?.patchBand(jwt, bandIdx, band)?.enqueue(object : Callback<PatchBandResponse> {
            override fun onResponse(call: Call<PatchBandResponse>, response: Response<PatchBandResponse>) {
                // 응답이 왔을 때 처리
                Log.d("PATCH / SUCCESS", response.toString())

                val resp : PatchBandResponse = response.body()!!
            ...
            override fun onFailure(call: Call<PatchBandResponse>, t: Throwable) {
                // 네트워크 연결이 실패했을 때 실행
