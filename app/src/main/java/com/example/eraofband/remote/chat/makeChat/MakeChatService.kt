...
class MakeChatService {
    ...
    fun makeChat(makeChatRoom: MakeChatRoom) {
        ...
        makeChatService?.makeChat(makeChatRoom)?.enqueue(object : Callback<MakeChatResponse> {
            override fun onResponse(call: Call<MakeChatResponse>, response: Response<MakeChatResponse>) {
                // 응답이 왔을 때 처리
                Log.d("MAKE CHAT / SUCCESS", response.toString())

                val resp : MakeChatResponse = response.body()!!
            ...
            override fun onFailure(call: Call<MakeChatResponse>, t: Throwable) {
                // 네트워크 연결이 실패했을 때 실행
