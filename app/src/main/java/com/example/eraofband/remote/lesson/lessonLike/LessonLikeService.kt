...
class LessonLikeService {
    ...
    fun lessonLikeDelete(jwt: String, lessonIdx : Int) {  // 좋아요 취소
        ...
        lessonLikeService?.lessonLikeDelete(jwt, lessonIdx)?.enqueue(object : Callback<LessonLikeDeleteResponse> {
            override fun onResponse(call: Call<LessonLikeDeleteResponse>, response: Response<LessonLikeDeleteResponse>) {
                // 응답이 왔을 때 처리
                Log.d("DELETELIKE / SUCCESS", response.toString())

                val resp : LessonLikeDeleteResponse = response.body()!!
            ...
            override fun onFailure(call: Call<LessonLikeDeleteResponse>, t: Throwable) {
                // 네트워크 연결이 실패했을 때 실행
