...
class DeleteBoardImgService {
    ...
    fun deleteBoardImg(boardImgIdx : Int) {
        ...
        deleteBoardImgService?.deleteBoardImg(boardImgIdx)?.enqueue(object :
            Callback<DeleteBoardImgResponse> {
            override fun onResponse(call: Call<DeleteBoardImgResponse>, response: Response<DeleteBoardImgResponse>) {
                // 응답이 왔을 때 처리
                Log.d("DELETE BOARD IMG / SUCCESS", response.toString())

                val resp : DeleteBoardImgResponse = response.body()!!
            ...
            override fun onFailure(call: Call<DeleteBoardImgResponse>, t: Throwable) {
                // 네트워크 연결이 실패했을 때 실행
